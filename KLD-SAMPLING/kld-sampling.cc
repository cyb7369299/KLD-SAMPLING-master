...
void kld_sampling::init(float quantile, float err, const vector<float>& bsz, int sample_min) {
  ...
  confidence=std::min(0.49998f,std::max(0.f,confidence));
...
int kld_sampling::update(const vector<float>& sample) {
  ...
  curr_sample=sample;   // get sample
  num_samples++;        //num_samples<int>

  if (in_empty_bin()) { // step 9 to 13 in papaer "Adapting the sample size in pf through kld sampling" TableII
    support_samples++;
