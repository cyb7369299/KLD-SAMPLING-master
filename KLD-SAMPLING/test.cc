...
// this is a point drawn from a 1D Gaussian distribution centered at
// mean with a given standard deviation. 's filter...
...
int main(int argc, char *argv[]) {
  ...
  quantile = std::min(0.99998f,quantile);
  ...
  cout << "KLD quantile: " << quantile << endl;             // 分位數 = 0.5f
  cout << "KLD error: " << kld_error << endl;               // error = 0.1f
  cout << "KLD bin size: " << bin_size << endl;             // bin_size = 0.1f
  cout << "Minimum # of samples: " << min_samples << endl;  // min<int> = 10
  cout << "Random Seed: "<<seed<<endl<<endl;                // seed = 0.1f            ???
  ...
  // step 3 to 15 in papaer "Adapting the sample size in pf through kld sampling" TableII
